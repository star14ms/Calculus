...
################################################################################ 입력 단계
  ...
  if Discrime_rational_num(y) == False: continue # 유리수 판별(y)
  # 유리수가 아니면 초기화
...
################################################################################ 정적분
  if c == '정적분' or c == '3': # 정적분을 골랐을 때
    ...
    if len(x) != 2 :continue
    if Discrime_rational_num(x) == False: continue # 유리수 판별(x)
    ...
    if Convertible_to_decimal(값.denominator) and 값.denominator != 1: # 소수변환가능(값.분모)
      print(float(값))
    else:
      print(값)
